        ...
        <div class="container py-5">
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    ...
                    $enviadasTabla = "SELECT * FROM enviados WHERE entregada = 1 AND id_trabajo = $idT ;";
                    $recibirTabla = $objConexion->consultar($enviadasTabla);
                    ?>

                    <form action="mostrarTareaP.php?id=<?php echo $idT?>" method="post">
                    <table class="table col-12" >
                      ...
                      <tbody>
                        <?php foreach($recibirTabla as $TR){

                            $archivoRuta = 'imagenes/';
                            $archivoT = $archivoRuta . $TR['archivo'];

                            $userA = $TR['id_alumno'];
                           
                            $nombreA = "SELECT * FROM usuario WHERE dni = $userA;";
                            $na = $objConexion->consultar($nombreA);
                            $name = $na[0]['nombre'];
                            $apellido = $na[0]['apellido'];
                        ?>
                        <tr>
                          <th scope="row">1</th>
                          <td><?php echo $name ." ". $apellido ;?></td>
                          <td><a href="<?php echo $archivoT;?>" download><?php 
                          
                          $nombreDoc = $TR['archivo'];
                                    $maxCaracteres = 15; // Cambia este valor al número deseado de caracteres
                                    if (strlen($nombreDoc) > $maxCaracteres) {
                                        echo substr($nombreDoc, 0, $maxCaracteres) . '...';
                                    } else {
                                        echo $nombreDoc;
                                    }
                          ...
                          ?></a></td>
                          <td>
                        <input type="number" style = "width: 10vh" name="nota" id="">
                          <input class="btn btn-primary" type="submit" value="Enviar"></td>
                        </tr>
                   ...
                   <?php }?>
                    ...
                    if($_SERVER['REQUEST_METHOD'] === 'POST'){
                        ...
                        $cargaNota = $objConexion->ejecutar("UPDATE enviados SET nota = $nota WHERE id_alumno = $userA AND id_trabajo = $idT;");
                        }
                        else echo "La nota debe estar en el rango de 1 y 10";
                    }?>
                    </form>
